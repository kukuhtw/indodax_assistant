...
| Log berisi `telegram.error.Conflict: terminated by other getUpdates request` | Ada **lebih dari satu proses** yang polling dengan `TELEGRAM_BOT_TOKEN` yang sama secara bersamaan. **Jika hanya muncul sesaat lalu hilang** saat redeploy/restart, itu normal (poller lama belum sempat menutup koneksi) — pastikan command container memakai `exec uvicorn ...` (bukan `uvicorn ...` tanpa `exec`) agar `SIGTERM` diteruskan dengan benar. **Jika muncul terus-menerus sejak container baru start**, berarti ada instance lain yang aktif di tempat lain saat ini juga: (1) cek `docker ps -a` di VPS untuk container duplikat lalu `docker stop`/`docker rm` yang tidak seharusnya ada; (2) cek Dokploy untuk deployment/service ganda dengan `.env` token yang sama; (3) cek apakah ada instance lokal/dev atau server lain yang masih jalan dengan token yang sama. Jika sumbernya tidak ditemukan, cara paling pasti: revoke token lewat @BotFather (`/mybots` → bot → API Token → Revoke current token), lalu perbarui `TELEGRAM_BOT_TOKEN` di `.env` pada deployment yang benar dan redeploy — ini langsung mematikan poller lain mana pun yang masih memegang token lama. |
